...
AUTHOR = "Lucas Pereira Walter"
ROLE = "Desenvolvedor de sites"
PORTFOLIO = "lucaspwalter.github.io/portfolio"
WHATSAPP = "(47) 99155-0977"
...
def footer(draw, number, accent, dark=False):
    fill = "#ffffff99" if dark else MUTED
    draw.text((75, H-72), f"{AUTHOR.upper()} • PROPOSTA DEMONSTRATIVA", font=font(15, True), fill=fill)
...
def build(model):
    ...
    d.text((75,775),"PROPOSTA COMERCIAL • SITE PROFISSIONAL",font=font(25,True),fill=accent)
    y=paragraph(d,f"Presença digital pensada para {name.lower()}.",75,850,font(66,True),WHITE,1060,1.08)
    y=paragraph(d,f"Conceito demonstrativo com identidade própria, informações claras e caminhos diretos para contato em qualquer tela.",75,y+50,font(28),"#cbd2d8",1000)
    d.text((75,y+48),f"{AUTHOR}  •  {ROLE}",font=font(21,True),fill=WHITE)
    ...
    paragraph(d,f"Um site próprio organiza serviços, diferenciais e canais de contato de {name.lower()}. O cliente encontra respostas rápidas e percebe uma marca mais preparada.",75,345,font(29),MUTED,1030)
    ...
    for number,title,body,file in [(3,"Primeira impressão que apresenta",f"Imagem exclusiva, mensagem direta e botões estratégicos comunicam a proposta de {name.lower()} em segundos.","desktop.png"),(4,"Pronto para o celular","Menu compacto, leitura confortável e chamadas de ação grandes. Estrutura adaptada sem cortes.","mobile.png")]:
        p=Image.new("RGB",(W,H),WHITE); d=ImageDraw.Draw(p); d.text((75,80),"EXPERIÊNCIA DO PROJETO",font=font(18,True),fill=accent); d.text((75,140),title,font=font(55,True),fill=INK)
    ...
    d.rounded_rectangle((75,1360,1165,1605),radius=24,fill=CREAM)
    d.text((115,1400),"Vamos conversar sobre o projeto?",font=font(34,True),fill=INK)
    d.text((115,1470),f"WhatsApp  {WHATSAPP}",font=font(24,True),fill=accent)
    d.text((115,1515),f"Portfólio  {PORTFOLIO}",font=font(22),fill=INK)
    paragraph(d,"Escopo, prazo e investimento definidos após entender as necessidades da empresa.",115,1560,font(19),MUTED,950)
    footer(d,5,accent,True); pages.append(p)
    SAIDA.mkdir(exist_ok=True)
    output = SAIDA / filename(name)
    pages[0].save(output,"PDF",resolution=150,save_all=True,append_images=pages[1:],title=f"Proposta comercial de site — {name}",author=AUTHOR,subject=f"Projeto demonstrativo de site para {name}")
for model in MODELOS: build(model)
expected = {filename(model["nome"]) for model in MODELOS}
for old in SAIDA.glob("*.pdf"):
